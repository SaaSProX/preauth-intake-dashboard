...
const CALLBACK_META = {
  delivered: { label: 'Delivered', cls: 'approve', help: 'Advisory decision was accepted by AMAN.' },
  skipped_disabled: { label: 'Disabled', cls: 'pending', help: 'Automatic AMAN write-back is disabled.' },
  skipped_no_config: { label: 'Needs config', cls: 'pending', help: 'AMAN endpoint/key is not configured.' },
  skipped_no_line_decisions: { label: 'No lines', cls: 'pending', help: 'No pending item-level decisions were available to send.' },
  auth_failed: { label: 'Auth failed', cls: 'deny', help: 'AMAN rejected the key.' },
  scope_missing: { label: 'Scope missing', cls: 'deny', help: 'AMAN key is missing pa.decisions.write or pa.* scope.' },
  stale_revision: { label: 'Stale revision', cls: 'escalate', help: 'AMAN rejected the decision because the submission revision is stale.' },
  network_error: { label: 'Network error', cls: 'error', help: 'Could not reach AMAN decision API.' },
};
function callbackMeta(status) {
  const s = String(status || '').toLowerCase();
  if (!s) return { label: 'Not sent', cls: 'pending', help: 'No advisory write-back attempt is recorded yet.' };
  if (s.startsWith('http_')) return { label: s.toUpperCase(), cls: 'error', help: 'AMAN returned a non-success HTTP response.' };
  return CALLBACK_META[s] || { label: s.replace(/_/g, ' '), cls: 'pending', help: 'Latest AMAN write-back state.' };
}
...
function mapRequest(r) {
  ...
  return {
    ...
    callback_status: r.callback_status || null,
    callback_http_status: r.callback_http_status ?? null,
    callback_sent_at: r.callback_sent_at || null,
...
function CallbackPill({ status, tipAlign }) {
  const m = callbackMeta(status);
  return <span className={`pill ${m.cls}`} data-tip={m.help} data-tip-align={tipAlign || 'right'}><span className="dot" />{m.label}</span>;
}
function Conf({ level }) {
...
function AmanWritebackSummary({ summary }) {
  const delivered = Number(summary.callback_delivered || 0);
  const failed = Number(summary.callback_failed || 0);
  const skipped = Number(summary.callback_skipped_disabled || 0)
    + Number(summary.callback_skipped_no_config || 0)
    + Number(summary.callback_skipped_no_line_decisions || 0);
  const notAttempted = Number(summary.callback_not_attempted || 0);
  const last = summary.callback_last_sent_at ? timeAgo(summary.callback_last_sent_at) : '—';
  const items = [
    ['Delivered', delivered, 'var(--ok)', 'Accepted by AMAN decision API.'],
    ['Failed', failed, failed ? 'var(--bad)' : 'var(--ink-4)', 'Auth, scope, stale revision, network, or HTTP failure.'],
    ['Skipped', skipped, 'var(--warn)', 'Disabled, missing config, or no pending line decisions.'],
    ['Not attempted', notAttempted, 'var(--ink-4)', 'Processed decisions with no write-back attempt recorded.'],
    ['Last attempt', last, 'var(--indigo)', 'Most recent write-back attempt in this filtered period.'],
  ];
  return (
    <div style={{ border: '1px solid var(--line)', borderRadius: 9, background: 'var(--bg-2)', padding: '12px 14px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 10 }}>
      {items.map(([label, value, color, tip]) => (
        <div key={label} data-tip={tip} data-tip-align="left" style={{ display: 'flex', alignItems: 'center', gap: 9, minWidth: 0 }}>
          <span className="led" style={{ background: color }} />
          <div style={{ minWidth: 0 }}>
            <div className="muted mono" style={{ fontSize: 10.5 }}>{label}</div>
            <div className="mono" style={{ fontSize: 13, color: 'var(--ink)' }}>{typeof value === 'number' ? value.toLocaleString() : value}</div>
          </div>
        </div>
      ))}
    </div>
...
function AmanWritebackPanel({ r, canSend, sending, onSend }) {
  const status = r.callback_status;
  const hasDecision = !!r.decision && !['pending', 'processing', 'received'].includes(r.status);
  const canClick = canSend && hasDecision && !sending;
  const sentAt = r.callback_sent_at ? `${new Date(parseApiDate(r.callback_sent_at)).toLocaleString()}` : '—';
  return (
    <div style={{ border: '1px solid var(--line)', background: 'var(--bg-2)', borderRadius: 9, padding: '12px 14px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
        <div>
          <div className="sec-h" style={{ margin: 0 }}>AMAN advisory write-back</div>
          <div className="muted mono" style={{ fontSize: 11, marginTop: 4 }}>
            Item-level advisory decisions sent back to AMAN for this PA.
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          <CallbackPill status={status} />
          {canSend ? (
            <button
              className="btn sm"
              disabled={!canClick}
              onClick={() => onSend?.(r.request_id)}
              data-tip={hasDecision ? 'Manually POST the latest item-level advisory decision to AMAN.' : 'Agent must finish a decision before it can be sent.'}
              data-tip-align="right"
            >
      ...
      <div className="dgrid" style={{ marginTop: 10 }}>
        <div className="cell"><div className="lab">HTTP</div><div className="val mono">{r.callback_http_status || '—'}</div></div>
        <div className="cell"><div className="lab">Last attempt</div><div className="val mono">{sentAt}</div></div>
        <div className="cell"><div className="lab">Status code</div><div className="val mono">{status || 'not_sent'}</div></div>
      </div>
      {r.callback_error ? (
        <div style={{ marginTop: 10, padding: '9px 10px', borderRadius: 8, background: 'var(--bad-bg)', color: 'var(--bad-ink)', border: '1px solid var(--bad-line)', fontFamily: 'var(--mono)', fontSize: 11.5 }}>
          {r.callback_error}
        </div>
      ) : null}
    </div>
...
function AppInner() {
  ...
  const [sendingDecisionIds, setSendingDecisionIds] = useState(() => new Set());
  ...
  async function sendAmanDecision(requestId) {
    if (!session?.token || !requestId) return;
    setWritebackNotice('');
    setWritebackError('');
    setSendingDecisionIds((current) => new Set([...current, requestId]));
    try {
      const res = await apiRequest('/auth/preauth/send-decision', {
        method: 'POST',
        body: {
          request_id: requestId,
          ...(viewOrgId ? { org_id: viewOrgId.id } : {}),
      ...
      const status = res?.callback?.status || res?.result?.status || 'attempted';
      const http = res?.callback?.http_status ? ` · HTTP ${res.callback.http_status}` : '';
      setWritebackNotice(`AMAN write-back ${status}${http} for ${requestId}`);
      await loadDashboard({ silent: true });
    } catch (err) {
      setWritebackError(err.message || 'Could not send AMAN decision');
    } finally {
      setSendingDecisionIds((current) => {
        const next = new Set(current);
        next.delete(requestId);
        return next;
      });
          ...
          <section id="view-intake">
            ...
            {activeTab === 'dashboard' ? (
              <div id="tab-dashboard" className="loading-host dashboard-loading-host">
                ...
                {writebackNotice ? <div className="ro-banner" style={{ display: 'flex', marginTop: 18, background: 'var(--ok-bg)', borderColor: 'var(--ok-line)', color: 'var(--ok-ink)' }}><span className="led" style={{ background: 'var(--ok)' }} /> {writebackNotice}</div> : null}
                {writebackError ? <div className="ro-banner" style={{ display: 'flex', marginTop: 18, background: 'var(--bad-bg)', borderColor: 'var(--bad-line)', color: 'var(--bad-ink)' }}><span className="led" style={{ background: 'var(--bad)' }} /> {writebackError}</div> : null}
                ...
                <div className="section-gap" style={{ marginTop: 16 }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 16, marginBottom: 10 }}>
                    <h2 style={{ fontFamily: 'var(--mono)', fontSize: 15, fontWeight: 500, margin: 0 }}>AMAN write-back</h2>
                    <span className="muted mono" style={{ fontSize: 11 }}>Advisory decision delivery</span>
                  </div>
                  <AmanWritebackSummary summary={summary} />
                </div>
